    ...
    const fetchProfiles = async () => {
      try {
        ...
        if (data.status === 'success') {
          ...
          const transformedProfiles: Profile[] = data.users.map((user: any) => ({
            id: user.uid,
            name: user.name,
            ...
            shortBio: 'Developer profile', // Default value since it's not in the API
    ...
    if (searchTerm) {
      result = result.filter(
        (profile) =>
          ...
          (profile.shortBio && profile.shortBio.toLowerCase().includes(searchTerm.toLowerCase())),
      )
    }
